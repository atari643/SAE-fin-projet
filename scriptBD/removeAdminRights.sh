...
if [ -n "$checkconnection" ]; then
    ...
    then
        user_exists=$(mysql --user=$username--password=$password --database=$database --host=$host --execute="SELECT COUNT(*) FROM user WHERE email='$email' and admin=1;" -s)
        if [ -n "$user_exists" ] && [ $user_exists -eq 1 ]; then
            mysql --user=$username--password=$password --database=$database --host=$host --execute="UPDATE user SET admin=0 WHERE email='$email';"
